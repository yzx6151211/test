...
class Get_selenium():
    ...
    def parse_downadd(self):
        ...
        #print(html)
        a = re.findall('''"fileid":"(.*?)".*?"name":"(.*?)"''', html)
        for i in a:
            if i[1][0:11] == self.file_name[0:11]:
                fid = i[0]
               # print(i[0])
               # print(i[1])
                break
        #print(a[0][0])
        #print(a[0][1])
        id_url = "https://www.kdocs.cn/3rd/drive/api/v3/groups/1040371001/files/"+fid+"/download?isblocks=false"
        parse_str = self.session.get(id_url, headers=self.headers)
        prase_con = parse_str.content.decode()
        parse_ret = json.loads(prase_con)
        return parse_ret['fileinfo']['url'],i[1]
...
class Xls_set:
    def __init__(self,file_name):
        ...
        self.excelBook3 = ol.load_workbook(file_name,data_only=True)
        ...
        for i in range(1, 9):
            ...
            if isinstance(cell, datetime.datetime): # 判断是不是日期，是的话转字符串
                ...
                time = (time[0:4]+"年"+time[5:7]+"月"+time[8:10]+"日")
        ...
        for a in range(1, 6):
            ...
            excelBook = ol.load_workbook(excelPath,data_only=True)
            name2 = excelBook.create_sheet('Sheet2')
            name2.cell(1, 1).value = "小份"
            name = excelBook['Sheet1']
            name.delete_rows(1,1)
            name3 = excelBook['Sheet1']
            name4 =  excelBook['Sheet2']
            # 插入三行
            ...
            name.cell(4, 6).value = "大份"
            name.cell(5, 6).value = "小份"
            name.cell(55, 6).value = "大份"
            name.cell(56, 6).value = "小份"
            e1 = 0  # 第一个大份总数
            e2 = 0  # 第一个小份总数
            e3 = 0  # 第二个大份总数
            e4 = 0  # 第二个大份总数
          ...
          # print(name_xls)
            ...
            max_row = name.max_row
            max_column = name.max_column
            ...
            for m in range(1, max_row + 1):
                ...
                for n in range(97, 97 + max_column):  # chr(97)='a'
                    ...
                    cell1 = name[i].value  # 获取data单元格数据

                    name2[i].value = cell1  # 赋值到test单元格

            name.delete_rows(52, 61) #删除表一6楼
            name2.delete_rows(1,51)
            name2.merge_cells("A1:C1")# 合并单元格

            kuang3 = 0
            for i in range(4, 9):
                p = (name4.cell(i, 4).value)
            ...
            kuang_list = ["a", "b", "c"]
            for k in kuang_list:
                for i in range(1, kuang3+4):
                    name2[k + str(i)].border = border_set

            name2.cell(2, 2).value ="所属楼层：6"

            name2.column_dimensions['A'].width = 10.25
            name2.column_dimensions['B'].width = 12.5
            name2.column_dimensions['C'].width = 8.25
            name2.column_dimensions['D'].width = 8.38
    ...
    def delete(self,file_name):
        ...
        for root, dirs, files in os.walk(path):
            for i in files:
                ...
                if i[0:2] == "1." or i[0:2] == "2." or i[0:2] == "3." or i[0:2] == "4." or i[0:2] == "5.":
